...
crs_idw <- st_crs(IDW_temp_data_17)  
...
if (crs_idw != crs_polygon) {
  IDW_temp_data_17 <- st_transform(IDW_temp_data_17, crs = crs_polygon)  # Transform IDW result to polygon's CRS
...
idw_clipped <- st_intersection(IDW_rain_data_18, BC_polygon)

# Create the map of the clipped results
ggplot(data = idw_clipped) +
  geom_sf(aes(fill = var1_pred), color = NA) +  
  scale_fill_viridis_c(option = "d") +  
  labs(title = "IDW Interpolation of Total Precipitation in BC (2018)",
...
ggsave("Clipped_IDW_Interpolation_2018_rain_Map.png", width = 10, height = 8, dpi = 300)
